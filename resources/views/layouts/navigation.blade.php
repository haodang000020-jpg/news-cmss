@if (request()->is('admin*'))
    @php
        $adminNavItems = [
            ...
            [
                ...
                'active' => 'admin.document-categories.*',
                'permission' => 'documents.manage',
            ],
            [
                'label' => 'Trang tĩnh',
    ...
    <nav class="admin-sidebar-inner">
        <a href="{{ route('admin.dashboard') }}" class="admin-brand">
            <span class="admin-brand-mark">CMS</span>
            <span>CMS Cổng thông tin</span>
        </a>

        <div class="admin-nav-section">Quản trị nội dung</div>

        <div class="admin-nav-list">
            @foreach ($adminNavItems as $item)
                @if (
                    \Illuminate\Support\Facades\Route::has($item['route'])
                    && ($item['permission'] === null || Auth::user()?->hasPermissionTo($item['permission']))
                )
                    <a href="{{ route($item['route']) }}" class="admin-nav-link {{ request()->routeIs($item['active']) ? 'is-active' : '' }}">
                        {{ $item['label'] }}
                    </a>
                @endif
            @endforeach
