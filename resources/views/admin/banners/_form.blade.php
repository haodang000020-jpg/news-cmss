...
@php
    $bannerPositions = array_replace($positions ?? [], [
        ...
        'propaganda_slider' => 'Banner tuyên truyền',
    ]);
@endphp
...
<div class="grid gap-6">
    <div>
        ...
        <input id="title" name="title" type="text" value="{{ old('title', $banner->title) }}"
    ...
    <div>
        ...
        <input id="image" name="image" type="file" accept=".jpg,.jpeg,.png,.webp"
            class="mt-1 block w-full text-sm text-gray-700" @required(!$banner->exists)>

        @if ($banner->image)
            <img src="{{ asset('storage/' . $banner->image) }}" alt="{{ $banner->title }}"
                class="mt-3 h-24 w-48 rounded-md object-cover">
        @endif
    ...
    <div>
        ...
        <input id="link" name="link" type="url" value="{{ old('link', $banner->link) }}"
    ...
    <div>
        ...
        <select id="position" name="position"
            ...
            @foreach ($bannerPositions as $value => $label)
                <option value="{{ $value }}" @selected(old('position', $banner->position) === $value)>
                    {{ $label }}
                </option>
            @endforeach
        </select>
    ...
    <div>
        ...
        <label class="inline-flex items-center gap-2 text-sm font-medium text-gray-700">
            <input name="is_active" type="checkbox" value="1" @checked(old('is_active', $banner->is_active))
